...
DROP PROCEDURE IF EXISTS `GetQuickiesById`;

SET NAMES utf8mb4;
SET collation_connection = 'utf8mb4_unicode_ci';

DELIMITER //
CREATE PROCEDURE `GetQuickiesById`(
	IN `inputQuickieIds` VARCHAR(255)
)
BEGIN
...
FROM `Quickies` 
    ...
    LEFT JOIN `Exercises` AS `e4` ON (`eId4`=`e4`.`eId`)
	LEFT JOIN `Videos` AS `v4` ON (`e4`.`vId`= `v4`.`vId`)
WHERE FIND_IN_SET(`qId`, `inputQuickieIds`);
...
DROP PROCEDURE IF EXISTS `GetAllQuickieWorkouts`;

SET NAMES utf8mb4;
SET collation_connection = 'utf8mb4_unicode_ci';

DELIMITER //
CREATE PROCEDURE `GetAllQuickieWorkouts`()
BEGIN

CALL `GetAllQuickies`();

SELECT `wId`, `wName`, `wDifficulty`, `bsName`, `qids`
FROM `QuickieWorkouts` LEFT JOIN `BodySplits` ON (`QuickieWorkouts`.`bsId`= `BodySplits`.`bsId`) 
ORDER BY `wDifficulty`, `wName`;
...
DROP PROCEDURE IF EXISTS `GetQuickieWorkouts`;

SET NAMES utf8mb4;
SET collation_connection = 'utf8mb4_unicode_ci';

DELIMITER //
CREATE PROCEDURE `GetQuickieWorkouts`(
	IN `inputDifficulties` VARCHAR(15),
	IN `inputBodySplitIds` VARCHAR(70)
)
BEGIN
...
SELECT `wId`, `wName`, `wDifficulty`, `bsName`, `qids`
FROM `QuickieWorkouts` LEFT JOIN `BodySplits` ON (`QuickieWorkouts`.`bsId`= `BodySplits`.`bsId`) 
WHERE FIND_IN_SET(`wDifficulty`, `inputDifficulties`) 
	AND FIND_IN_SET(`QuickieWorkouts`.`bsId`, `inputBodySplitIds`) 
ORDER BY `wDifficulty`, `wName`;
